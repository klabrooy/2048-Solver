...
int main(int argc, char *argv[]) {
	...
	//count for nodes expanded, generated
	int generated = 0;
	int expanded = 0;
	...
	while (true) {
		 ...
		 */
		move_t selected_move = get_next_move( board, max_depth, propagation, &generated, &expanded);
	...
	double sec = (msec%1000)/1000;
	double expPerSec = expanded/sec;


	//find max tile on final board
	int i, j;
	int maxTile = 2;
	for (i = 0; i < SIZE; i++) {
		for (j = 0; j < SIZE; j++) {
			if ((int)getTile(board, i, j) > maxTile) {
				maxTile = (int)getTile(board, i, j);
			}
		}
	}
  ...
  fprintf(fp, "Expanded: %d\n", expanded);
	...
	fprintf(fp, "Node expanded per second: %f\n", expPerSec);
	fprintf(fp, "Time taken: %f\n", sec);
	fprintf(fp, "Max Tile: %d\n", maxTile);
	fprintf(fp, "Score: %d\n", score);
